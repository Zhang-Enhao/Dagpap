...
# 加载和预处理数据集
dataset = load_dataset("web_of_science", 'WOS46985', split='train')
...
def tokenize_and_encode(examples):
    inputs = sciBERT_tokenizer(examples['input_data'],padding=True, truncation=True, max_length=512,return_tensors="pt")
    ...
    labels = []
    org_token = sciBERT_tokenizer.decode(sciBERT_tokenizer.encode(examples["input_data"], add_special_tokens=False)).split(" ")
...
for exmples in dataset:
    inputs = tokenize_and_encode(exmples)
    new_data["input_ids"].append(inputs["input_ids"][0].numpy())
    new_data["labels"].append(inputs["labels"][0].numpy())
    new_data["attention_mask"].append(inputs["attention_mask"][0].numpy())
    new_data["token_type_ids"].append(inputs["token_type_ids"][0].numpy())
df = pd.DataFrame(data=new_data)
df.to_parquet( 'pretrain_data.parquet',engine='fastparquet')
